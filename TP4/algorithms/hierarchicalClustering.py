...
class ClusterNode:
    ...
    # cluster 
    def getMaxDistance(self, other, D):
        return max([max([np.linalg.norm(D[i] - D[j]) for j in other.nodes]) for i in self.nodes])
...
class HierarchicalClustering:
    ...
    def group(self, D):
        ...
        while len(groups) > 1:
            distances = [ (x, *min([(x.getMaxDistance(other, D), other) for other in groups if other != x], key=lambda x: x[0])) for x in groups]
            minPair = min(distances, key=lambda x: x[1])
            newNode = ClusterNode(children=[minPair[0], minPair[2]], nodes=[*minPair[0].nodes, *minPair[2].nodes])
            minPair[0].p = newNode
            minPair[2].p = newNode
            groups.remove(minPair[0])
            groups.remove(minPair[2])
